...
from diffi_sklearn_mod_functions import *
...
def local_diffi(iforest, x):
    ...
    # initialization 
    estimators = iforest.estimators_
    cfi = np.zeros(x.shape[-1]).astype('float')
    counter = np.zeros(x.shape[-1]).astype('int')
...
def local_diffi_batch(X: np.array, model:Type[IsolationForest]=IsolationForest()):
    ...
    for i in range(X.shape[0]):
        x_curr = X[i, :]
        fi_curr = model.local_diffi(x_curr)
        fi.append(fi_curr)
