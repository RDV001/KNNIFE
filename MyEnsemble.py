...
class MyIF(DIF):
    ...
    def decision_function(self, X):
        return invert_order(
            self.iForest_lst[0].decision_function(
                self._deep_representation(self.net_lst[0], X)
            )
        )
...
class MyEIF(DIF):
    ...
    def reduce_representation(self):
        nodes = np.sum(
            [
                self.iForest_lst[0].estimators_[j].tree_.node_count
                for j in range(len(self.iForest_lst[0].estimators_))
            ]
        )
