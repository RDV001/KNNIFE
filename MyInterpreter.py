...
class MyInterpreter:
    ...
    def _LFI(self, forest, h, x):
        y = self.ensemble._deep_representation(
            self.ensemble.net_lst[h], x.reshape(1, -1)
        ).reshape(1, -1)
        ...
        for k, estimator in enumerate(forest.estimators_):
            ...
            for i in range(len(path) - 1):
                ...
                i = (
                    self.is_left[h, k, path[i]]
                    if path[i + 1] == children_left[path[i]]
                    else self.is_right[h, k, path[i]]
                )
                V = V + v
                I = I + i
    ...
    def _GFI(self, forest, h):
        if self.method == "DIFFI":
            return diffi_interpretability_module.diffi_ib(
                forest, self.ensemble.x_reduced_lst[h]
            )
        ...
        for k, estimator in enumerate(estimators):
            # update I_O and V_O
            node_indicator_all_points_outliers = estimator.decision_path(
                Y_outliers_ib
            ).toarray()
            ...
            for i in range(len(Y_outliers_ib)):
                ...
                for i in range(len(path) - 1):
                    ...
                    I_O = I_O + i
            # update I_I and V_I
            node_indicator_all_points_inliers = estimator.decision_path(
                Y_inliers_ib
            ).toarray()
    ...
    def compute_exiffi_vs_is(self):
        ...
        self.vs = np.zeros(
            shape=(ensemble_num, estimators_num, max_nodes, vect_length)
        ).astype("float")
        self.is_left = np.zeros(
            shape=(ensemble_num, estimators_num, max_nodes, vect_length)
        ).astype("float")
        self.is_right = np.zeros(
            shape=(ensemble_num, estimators_num, max_nodes, vect_length)
        ).astype("float")
            ...
            for k, estimator in enumerate(forest.estimators_):
                for node in range(estimator.tree_.node_count):
                    if estimator.tree_.children_left[node] > -1:
                        v = np.abs(
                            self.ensemble.net_lst[h][
                                :, estimator.tree_.feature[node]
                            ].reshape(-1)
                        )
                        i_left = (
                            v
                            * estimator.tree_.n_node_samples[node]
        ...
        for h, forest in enumerate(self.ensemble.iForest_lst):
            ...
            for k, estimator in enumerate(forest.estimators_):
                ...
                node_indicator = estimator.decision_path(
                    self.ensemble.x_reduced_lst[h][in_bag_samples[k]]
                ).toarray()
                ...
                for node in range(node_count):
                    if children_left[node] > -1:
                        left_data = X_ib[
                            np.where(node_indicator[:, children_left[node]])[0]
                        ]
                        right_data = X_ib[
                            np.where(node_indicator[:, children_right[node]])[0]
                        ]
                        ...
                        left_nn_index = np.argmin(
                            distances, axis=1
                        )  # nearest right neighbor for each left point
                        right_nn_index = np.argmin(
                            distances, axis=0
                        )  # nearest left neighbor for each right point

                        # 3. Compute difference vectors to nearest neighbors
                        left_d = (
                            left_data - right_data[left_nn_index]
                        )  # shape: (n_left, n_features)
                        right_d = (
                            right_data - left_data[right_nn_index]
                        )  # shape: (n_right, n_features)
                        ...
                        i_left = (
                            v
                            * n_node_samples[node]
                            / n_node_samples[children_left[node]]
                        )
                        i_right = (
                            v
                            * n_node_samples[node]
                            / n_node_samples[children_right[node]]
                        )
            ...
            for k, estimator in enumerate(forest.estimators_):
                ...
                for node in range(node_count):
                    if children_left[node] > -1:
                        ...
                        # Shape: (n_left, n_right, n_features)
                        diffs = (
                            left_data[:, np.newaxis, :] - right_data[np.newaxis, :, :]
                        )
